...
function tibber(dependencies) {
    ...
    async function getPriceInfoCached() {
        ...
        log(`last price info entry is for day ${lastPriceInfoDay.format()}`);
        ...
        if (lastPriceInfoDay < tomorrow) {
            log(`Last price info entry is before tomorrow. Re-fetch prices immediately.`);
            priceInfoNextHours = getPriceInfo();
            return priceInfoNextHours;
        }
        ...
        log(`Expected price publish time is after ${expectedPricePublishTime.format()}`);
        if (now > expectedPricePublishTime) {
            ...
            setTimeout(async () => {
                priceInfoNextHours = await getPriceInfo()
            }, delay * 1000);
            return priceInfoNextHours;
        }
