...
export class TibberApi {
    ...
    constructor(log: Logger, homeySettings: ManagerSettings, homeId?: string, token?: string) {
        this.#log = log;
        this.#homeySettings = homeySettings;
        this.#token = token;
        this.#homeId = homeId;
        this.#log(`Initialize Tibber client for home ${homeId} using token ${token}`);
    }

    #getClient(): GraphQLClient {
        if (this.#token === undefined)
            this.#token = this.getDefaultToken();
        if (this.#token === undefined)
    ...
    async getPriceInfoCached() {
        ...
        const last = _.last(this.#priceInfoNextHours) as any;
        ...
        if (lastPriceInfoDay < tomorrow) {
            this.#log(`Last price info entry is before tomorrow. Re-fetch prices immediately.`);
            this.#priceInfoNextHours = await this.#getPriceInfo();
            return this.#priceInfoNextHours;
        }
        ...
        this.#log(`Expected price publish time is after ${expectedPricePublishTime.format()}`);
        if (now > expectedPricePublishTime) {
            const delay = getRandomDelay(0, 50 * 60);
            this.#log(`Last price info entry is before tomorrow and current time is after 13:00. Schedule re-fetch prices after ${delay} seconds.`);
            setTimeout(async () => {
                this.#priceInfoNextHours = await this.#getPriceInfo()
            }, delay * 1000);
            return this.#priceInfoNextHours;
    ...
    async #getPriceInfo(): Promise<PriceInfo[]> {
        ...
        if (priceInfoToday && priceInfoTomorrow)
            ...
            this.#priceInfoNextHours = priceInfoToday;
    ...
    subscribeToLive(callback: (result: any) => Promise<void>) {
