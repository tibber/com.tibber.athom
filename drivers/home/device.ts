...
class HomeDevice extends Device {
    ...
    #lastPrice?: any; // TODO
    #lastTemperature?: any; // TODO
    #location!: { lat: any, lon: any }; // TODO
    ...
    async onInit() {
        ...
        this.#insightId = this.#deviceLabel.replace(/[^a-z0-9]/ig,'_').toLowerCase();
        this.#lastPrice = undefined;
        this.#lastTemperature = undefined;
        const { latitude: lat, longitude: lon } = data.address;
        this.#location = { lat, lon };

        this.#priceChangedTrigger = this.homey.flow
            .getDeviceTriggerCard('price_changed')
            ;//.registerRunListener(() => true);

        this.#temperatureChangedTrigger = this.homey.flow
            .getDeviceTriggerCard('temperature_changed')
            ;//.registerRunListener(() => true);

        this.#consumptionReportTrigger = this.homey.flow
            .getDeviceTriggerCard('consumption_report')
            ;//.registerRunListener(() => true);

        this.#priceBelowAvgTrigger = this.homey.flow.getDeviceTriggerCard('price_below_avg');
        this.#priceBelowAvgTrigger.registerRunListener(this.#priceAvgComparer.bind(this));

        this.#priceAboveAvgTrigger = this.homey.flow.getDeviceTriggerCard('price_above_avg');
        this.#priceAboveAvgTrigger.registerRunListener(this.#priceAvgComparer.bind(this));

        this.#priceBelowAvgTodayTrigger = this.homey.flow.getDeviceTriggerCard('price_below_avg_today');
        this.#priceBelowAvgTodayTrigger.registerRunListener(this.#priceAvgComparer.bind(this));

        this.#priceAboveAvgTodayTrigger = this.homey.flow.getDeviceTriggerCard('price_above_avg_today');
        this.#priceAboveAvgTodayTrigger.registerRunListener(this.#priceAvgComparer.bind(this));

        this.#priceAtLowestTrigger = this.homey.flow.getDeviceTriggerCard('price_at_lowest');
        this.#priceAtLowestTrigger.registerRunListener(this.#priceMinMaxComparer.bind(this));

        this.#priceAtHighestTrigger = this.homey.flow.getDeviceTriggerCard('price_at_highest');
        this.#priceAtHighestTrigger.registerRunListener(this.#priceMinMaxComparer.bind(this));
        ...
        this.#priceAmongLowestTrigger.registerRunListener(this.#priceMinMaxComparer.bind(this));

        this.#priceAmongHighestTrigger = this.homey.flow.getDeviceTriggerCard('price_among_highest_today');
        this.#priceAmongHighestTrigger.registerRunListener(this.#priceMinMaxComparer.bind(this));

        this.#currentPriceBelowCondition = this.homey.flow.getConditionCard('current_price_below');
        this.#currentPriceBelowCondition.registerRunListener((args, _state) => args.price > this.#lastPrice.total);

        this.#currentPriceBelowAvgCondition = this.homey.flow.getConditionCard('cond_price_below_avg');
        this.#currentPriceBelowAvgCondition.registerRunListener(args => this.#priceAvgComparer(args, { below: true }));

        this.#currentPriceAboveAvgCondition = this.homey.flow.getConditionCard('cond_price_above_avg');
        this.#currentPriceAboveAvgCondition.registerRunListener(args => this.#priceAvgComparer(args, { below: false }));

        this.#currentPriceBelowAvgTodayCondition = this.homey.flow.getConditionCard('cond_price_below_avg_today');
        this.#currentPriceBelowAvgTodayCondition.registerRunListener(args => this.#priceAvgComparer(args, { below: true }));

        this.#currentPriceAboveAvgTodayCondition = this.homey.flow.getConditionCard('cond_price_above_avg_today');
        this.#currentPriceAboveAvgTodayCondition.registerRunListener(args => this.#priceAvgComparer(args, { below: false }));

        this.#currentPriceAtLowestCondition = this.homey.flow.getConditionCard('cond_price_at_lowest');
        this.#currentPriceAtLowestCondition.registerRunListener(args => this.#priceMinMaxComparer(args, { lowest: true }));

        this.#currentPriceAtHighestCondition = this.homey.flow.getConditionCard('cond_price_at_highest');
        this.#currentPriceAtHighestCondition.registerRunListener(args => this.#priceMinMaxComparer(args, { lowest: false }));

        this.#currentPriceAtLowestTodayCondition = this.homey.flow.getConditionCard('cond_price_at_lowest_today');
        this.#currentPriceAtLowestTodayCondition.registerRunListener(args => this.#priceMinMaxComparer(args, { lowest: true }));

        this.#currentPriceAtHighestTodayCondition = this.homey.flow.getConditionCard('cond_price_at_highest_today');
        this.#currentPriceAtHighestTodayCondition.registerRunListener(args => this.#priceMinMaxComparer(args, { lowest: false }));

        this.#currentPriceAmongLowestTodayCondition = this.homey.flow.getConditionCard('cond_price_among_lowest_today');
        this.#currentPriceAmongLowestTodayCondition.registerRunListener(args => this.#priceMinMaxComparer(args, { lowest: true }));

        this.#currentPriceAmongHighestTodayCondition = this.homey.flow.getConditionCard('cond_price_among_highest_today');
        this.#currentPriceAmongHighestTodayCondition.registerRunListener(args => this.#priceMinMaxComparer(args, { lowest: false }));

        this.#outdoorTemperatureBelowCondition = this.homey.flow.getConditionCard('temperature_below');
        this.#outdoorTemperatureBelowCondition.registerRunListener(args => args.temperature > this.#lastTemperature);

        this.#sendPushNotificationAction = this.homey.flow.getActionCard('sendPushNotification');
        this.#sendPushNotificationAction.registerRunListener(args => this.#tibber.sendPush(args.title, args.message));
    ...
    onDeleted() {
        ...
        this.homey.settings.set(`${this.#insightId}_lastLoggedDailyConsumption`, undefined);
        this.homey.settings.set(`${this.#insightId}_lastLoggerHourlyConsumption`, undefined);
    ...
    async getTemperature() {
        try {
            ...
            if (temperature && temperature !== this.#lastTemperature)
            {
                ...
                let temperatureLogger = await this.#createGetLog(`${this.#insightId}_temperature`, {
            ...
            console.error(`Error fetching weather forecast (${this.#location.lat},${this.#location.lon})`, e);
        }
    }
        ...
        try {
            ...
            if (this.isConsumptionReportEnabled()) {
                ...
                if (!lastLoggedDailyConsumption || !lastLoggedHourlyConsumption) {
                    const consumptionData = await this.#tibber.getConsumptionData(daysToFetch, hoursToFetch);
    ...
    async onPriceData(priceInfoNextHours: PriceInfo[]) {
        ...
        if (_.get(priceInfoCurrent, 'startsAt') !== _.get(this.#lastPrice, 'startsAt')) {
            ...
            if (priceInfoCurrent.total !== null) {
                ...
                let priceLogger = await this.#createGetLog(`${this.#insightId}_price`, {
                ...
                if (priceInfoNextHours !== undefined) {
                    this.#priceBelowAvgTrigger.trigger(this, undefined, { below: true }).catch(console.error);
                    this.#priceBelowAvgTodayTrigger.trigger(this, undefined, { below: true }).catch(console.error);
                    this.#priceAboveAvgTrigger.trigger(this, undefined, { below: false }).catch(console.error);
                    this.#priceAboveAvgTodayTrigger.trigger(this, undefined, { below: false }).catch(console.error);
                    this.#priceAtLowestTrigger.trigger(this, undefined, { lowest: true }).catch(console.error);
                    this.#priceAtHighestTrigger.trigger(this, undefined, { lowest: false }).catch(console.error);
                    this.#priceAmongLowestTrigger.trigger(this, undefined, { lowest: true }).catch(console.error);
                    this.#priceAmongHighestTrigger.trigger(this, undefined, { lowest: true }).catch(console.error);

                    if (this.#priceMinMaxComparer({}, { lowest: true }))
                        this.#priceAtLowestTodayTrigger.trigger(this, undefined, { lowest: true }).catch(console.error);
                    if (this.#priceMinMaxComparer({}, { lowest: false }))
                        this.#priceAtHighestTodayTrigger.trigger(this, undefined, { lowest: false }).catch(console.error);
                }
            }
        }
    }
    ...
    getLastLoggedDailyConsumption() {
        return this.homey.settings.get(`${this.#insightId}_lastLoggedDailyConsumption`);
    }

    setLastLoggedDailyConsumption(value: any) {
        this.homey.settings.set(`${this.#insightId}_lastLoggedDailyConsumption`, value);
    }

    getLastLoggedHourlyConsumption() {
        return this.homey.settings.get(`${this.#insightId}_lastLoggerHourlyConsumption`);
    }

    setLastLoggedHourlyConsumption(value: any) {
        this.homey.settings.set(`${this.#insightId}_lastLoggerHourlyConsumption`, value);
    }

    async onConsumptionData(data: any) {
        try {
            const lastLoggedDailyConsumption = this.getLastLoggedDailyConsumption();
            const consumptionsSinceLastReport: any[] = [];
            const dailyConsumptions = _.get(data, 'viewer.home.daily.nodes') || [];
            await mapSeries(dailyConsumptions, async (dailyConsumption: Consumption) => {
                if (dailyConsumption.consumption !== null) {
                    ...
                    let consumptionLogger = await this.#createGetLog(`${this.#insightId}_dailyConsumption`, {
                    ...
                    let costLogger = await this.#createGetLog(`${this.#insightId}_dailyCost`, {
        ...
        try {
            ...
            await mapSeries(hourlyConsumptions, async (hourlyConsumption: Consumption) => {
                if (hourlyConsumption.consumption !== null) {
                    ...
                    let consumptionLogger = await this.#createGetLog(`${this.#insightId}hourlyConsumption`, {
                    ...
                    let costLogger = await this.#createGetLog(`${this.#insightId}_hourlyCost`, {
    ...
    #priceAvgComparer({ hours, percentage }: { hours: number, percentage: number }, { below }: { below: boolean }): boolean {
        if (hours === 0)
            return false;

        const now = moment();
        let avgPriceNextHours: any;
        if (hours)
            avgPriceNextHours = _(this.#priceInfoNextHours)
                .filter(p => hours > 0 ? moment(p.startsAt).isAfter(now) : moment(p.startsAt).isBefore(now))
                .take(Math.abs(hours))
                .meanBy(x => x.total);
        else
            avgPriceNextHours = _(this.#priceInfoNextHours)
        ...
        let diffAvgCurrent = (this.#lastPrice.total - avgPriceNextHours) / avgPriceNextHours * 100;
        if (below)
            diffAvgCurrent = diffAvgCurrent * -1;

        this.log(`${this.#lastPrice.total.toFixed(2)} is ${diffAvgCurrent.toFixed(2)}% ${below ? 'below' : 'above'} avg (${avgPriceNextHours.toFixed(2)}) ${hours ? 'next ' + hours + ' hours' : 'today'}. Condition of min ${percentage} percentage met = ${diffAvgCurrent > percentage}`);
        return diffAvgCurrent > percentage;
    }

    #priceMinMaxComparer(options: { hours?: number, ranked_hours?: number }, { lowest }: { lowest: boolean }) {
        if (options.hours === 0 || options.ranked_hours === 0)
            return false;

        const now = moment();
        let pricesNextHours = options.hours !== undefined ?
            _(this.#priceInfoNextHours)
                .filter(p => options.hours! > 0 ? moment(p.startsAt).isAfter(now) : moment(p.startsAt).isBefore(now))
        ...
        if (options.ranked_hours !== undefined) {
            ...
            const currentHourRank = _.findIndex(pricesNextHours, ['startsAt', this.#lastPrice.startsAt]);
            if (currentHourRank < 0) {
                this.log(`Could not find the current hour rank among today's hours`);
                return false
            }

            conditionMet = lowest ? currentHourRank < options.ranked_hours
                : currentHourRank >= sortedHours.length - options.ranked_hours

                this.log(`${this.#lastPrice.total.toFixed(2)} is among the ${lowest ? 'lowest' : 'highest'} ${options.ranked_hours} hours today = ${conditionMet}`);
        } else {
            ...
            this.log(`${this.#lastPrice.total.toFixed(2)} is ${lowest ? 'lower than the lowest' : 'higher than the highest'} (${toCompare}) ${options.hours ? 'among the next ' + options.hours + ' hours' : 'today'} = ${conditionMet}`);
        }
